...
svg.append("text")
    ...
    .attr("x", width)
    .attr("y", height - 6)
    .text("ingreso percápita, ajustado a la inflación (dólares)");
// Add a y-axis label.
svg.append("text")
    .attr("class", "y label")
    .attr("text-anchor", "end")
...
d3.json("nations.json", function(nations) {
	...
	function init() {
		dragit.init(".gRoot");
		dragit.time = {min:1809, max:2017, step:1, current:1809}
		dragit.data = d3.range(nations.length).map(function() { return Array(); })
		for(var yy = 1809; yy<2017; yy++) {
	...
	function play_demo(pais,tiempo) {
  		var ex_nations = ["China", "India", "Indonesia", "Italia", "Francia", "España", "Alemania", "Estados Unidos"];
		var index_random_nation = null;
		var random_nation = nations.filter(function(d, i) { 
			if(d.name == ex_nations[pais]) {
		...
		update(tiempo,500);								
		dragit.utils.animateTrajectory(dragit.lineTrajectory, dragit.time.min, 20)
